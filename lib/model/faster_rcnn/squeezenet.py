...
class squeezenet(_fasterRCNN):
    def __init__(self,
                 ...
                 lighthead=False):
        self.model_path = 'data/pretrained_model/squeezenet{}.pth'.format(
            version)
    ...
    def _init_modules(self):
        ...
        if self.pretrained:
            ...
            squeezenet.load_state_dict({
                k: v
                for k, v in state_dict.items() if k in squeezenet.state_dict()
            })

        squeezenet.classifier = nn.Sequential(
            *list(squeezenet.classifier._modules.values())[:-1])

        # not using the last maxpool layer
        if self.lighthead:
            self.RCNN_base = nn.Sequential(
                *list(squeezenet.features._modules.values())[:self.clip])
        else:
            self.RCNN_base = nn.Sequential(
        ...
        if self.lighthead:
            self.lighthead_base = nn.Sequential(
                *list(squeezenet.features._modules.values())[self.clip + 1:])
