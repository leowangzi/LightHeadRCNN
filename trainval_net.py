...
from model.faster_rcnn.mobilenet_official import mobilenetv2_o
...
if __name__ == '__main__':
    ...
    if args.dataset == "pascal_voc":
        ...
        if cfg.TRAIN.SCALES >= 800:
            args.set_cfgs = [
                'ANCHOR_SCALES', '[32, 64, 128, 256, 512]', 'ANCHOR_RATIOS',
    ...
    elif args.net == 'mobilenet_o':
        _RCNN = mobilenetv2_o(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic, lighthead=lighthead)
    ...
    if args.resume:
        if args.lighthead:
            load_name = os.path.join(
                output_dir,
                'lighthead_rcnn_{}_{}_{}.pth'.format(args.checksession,
                                                     args.checkepoch,
                                                     args.checkpoint))
        else:
            load_name = os.path.join(
        ...
        if args.mGPUs:
            if args.lighthead:
                save_name = os.path.join(
                    output_dir, 'lighthead_rcnn_{}_{}_{}.pth'.format(
                        args.session, epoch, step))
            else:
                save_name = os.path.join(
                    output_dir,
                    'faster_rcnn_{}_{}_{}.pth'.format(args.session, epoch,
                                                      step))
            save_checkpoint(
                {
